...
info.cell(row=2, column=1).value = "This file contains the lists for the different substance classes. Please fill in the information for each substance in the corresponding sheet."
info.cell(row=2, column=1).font = Font(size=18, bold=True)
info.row_dimensions[2].height = 30
info.cell(row=3, column=1).value = "The following substance classes are available:"
...
info.cell(row=12, column=1).value = "The templates for the substance datasheets are in a separate file in the templates folder"
info.cell(row=12, column=1).font = Font(size=14, bold=True)
info.row_dimensions[12].height = 30
info.cell(row=13, column=1).value = '=HYPERLINK("../datatemplates/SubstanceDatasheetTemplate.xlsx", "Substance datasheet template")'
drawing = Image("OverviewOfClassesInTheModel.png")
info.add_image(drawing, "B9")

# Define the standard columns for all sheets
columns = ['name', 'code', "substance_type", 'criticality', "WasteStreams", 'code_EC','code_UNSPC', 'code_CAS', "prod_category", "prod_sub_category", "ISIC_name", "ISIC", "CPC_name", "CPC", "code_EI", 'tags', 'notes' ,'datasheet']
...
wb.save("data/ListOfAllSubstances.xlsx")
wb.close()