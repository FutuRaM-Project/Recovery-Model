...
if not os.path.exists('../flowcharts_graphvis/ortho/html'):
    os.makedirs('../flowcharts_graphvis/ortho/html')
# make folder for the dot files
if not os.path.exists('../flowcharts_graphvis/ortho/dot'):
    os.makedirs('../flowcharts_graphvis/ortho/dot')
# make folder for the pdf files
if not os.path.exists('../flowcharts_graphvis/ortho/pdf'):
    os.makedirs('../flowcharts_graphvis/ortho/pdf')
...
for WS in WSs:
    ...
    # Add nodes to the graph
        ...
        for _, row in df_ws.iterrows():
            if 'outsideSB' in row['destination level two']:
                c.edge(row['origin level two'], row['destination level two'], color='red', style='dotted')
            else:
                c.edge(row['origin level two'], row['destination level two'])
    ...
    g.attr(rankdir='LR', nodesep='0.5', ranksep='0.5', margin='0.2', splines='ortho')
    ...
    g.save(f'../flowcharts_graphvis/ortho/dot/graphvis_{WS}.dot')
    # Render the graph as a PDF
    g.render(f'../flowcharts_graphvis/ortho/pdf/graphvis_{WS}', view=False, cleanup=True, format='pdf')
...
# Render the graph as an SVG image
    ...
    with open(f'../flowcharts_graphvis/ortho/html/graphvis_{WS}.html', 'w') as f:
        f.write('<html>\n')
        f.write('<body>\n')
