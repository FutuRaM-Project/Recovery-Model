...
shred = Process(
    "Car shredder",
    "This is a unit process that represents the shredding of a car",
    ["physical", "shredding"],
)
collection = Process(
    "Collection",
    "This is a unit process that represents the collection of a car",
    ["physical", "collection"],
)

# add some flows to get the stuff back out
shred.waste_stream = "ELV"


flow_CarToShred = Flow(
    "car for shredding", to=shred, from_=collection, amount=1, unit="car"
)
flow_CarToShred.composition = car.composition

flow_CarShredded_metal = Flow(
    "metal from shredded car", to="X", from_=shred, amount=900, unit="kg"
)
...
flow_CarShredded_metal.amount = (
    0.9  # should be: amount of metal in the car * transfer coefficient
)

flow_CarShredded_metal.to_dict()

flow_CarShredded_plastic = Flow(
    "shredded car plastic", to="Y", from_=shred, amount=100, unit="kg"
)
...
# Create a process with description and tags
recycle_steel = Process(
    "Steel recycling",
    "This is a unit process that represents the recycling of steel",
    ["physical", "recycling", "pyrometallurgy"],
)
ocean_dumping = Process(
    "Ocean dumping",
    "This is a unit process that represents the dumping of plastic in the ocean",
    ["physical", "dumping", "ocean"],
)

# Create flows
flow_CarShredded_metal = Flow(
    "steel_from_shredder", to=recycle_steel, from_=shred, amount=1, unit="kg"
)
flow_CarShredded_metal.composition = {"Fe": 0.9, "C": 0.1}


flow_CarShredded_plastic = Flow(
    "plastic_from_shredder", to=ocean_dumping, from_=shred, amount=1, unit="kg"
)
flow_CarShredded_plastic.composition = plastic.composition

flow_recycled_steel = Flow(
    "recycled_steel", to="X", from_=recycle_steel, amount=0.9, unit="kg"
)
...
parameter_1 = Parameter(
    "Parameter 1",
    description="This parameter represents the conversion efficiency of the process.",
)
...
parameter_2 = Parameter(
    "Parameter 2",
    description="This parameter represents the energy consumption of the process.",
)
...
print(
    recycle_steel.description
)  # Output: This is a unit process that converts raw material to finished product.
...
)  # Output: This parameter represents the conversion efficiency of the process.

for obj in [
    recycle_steel,
    ocean_dumping,
    flow_CarShredded_metal,
    flow_CarShredded_plastic,
    flow_recycled_steel,
    parameter_1,
    parameter_2,
]:
