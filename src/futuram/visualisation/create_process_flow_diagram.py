...
def create_process_flow_diagram(model, process_filters=None, flow_filters=None, process_tag_filters=None,
    ...
    for process_name, process in model.processes.items():
        ...
        for input_flow in process.inputs:
            ...
            #     graph.node(matter_name, shape='ellipse', color='lightblue', fontcolor='black', style='filled',
            #                label=f"{matter_name}\nComponents: {components}\nElements: {elements}")
            #     graph.edge(matter_name, process_name, color='blue')
        ...
        for output_flow in process.outputs:
            ...
            #     graph.node(matter_name, shape='ellipse', color='lightblue', fontcolor='black', style='filled',
            #                label=f"{matter_name}\nComponents: {components}\nElements: {elements}")
            #     graph.edge(process_name, matter_name, color='blue')
