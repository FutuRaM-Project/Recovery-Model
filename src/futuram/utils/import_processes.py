...
import openpyxl
from futuram.classes import Process
...
def import_processes_xlsx(filename, model):
    ...
    """
    print(f'\n{"-" * 60}\n   Importing processes to model \"{model.name}\" \n   from {filename}\n{"-" * 60}\n')
    ...
    worksheet = workbook.worksheets[sheet_names.index('Level3')]
    # Get the headers
    ...
    for process in data:
        ...
        # columns from existing sheet
        new_process = Process(process['level 3 processes (technology)'])
        new_process.name_leveltwo = process['level 2 processes']
        new_process.name_levelone = process['level 1 processes']
        new_process.code = process['code 3']
        new_process.code_leveltwo = process['code 2']
        new_process.code_levelone = process['code 1']
        new_process.note = process['note']
        new_process.description = process['description']
        
        # derive WS from note
        WSs = ["BATT", "CDW", "MIN", "SLASH", "WEEE", "ELV"]
        process_WSs = [WS for WS in WSs if WS in process["note"]]
        new_process.WS = process_WSs
